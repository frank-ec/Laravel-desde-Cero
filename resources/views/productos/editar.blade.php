    ...
    <form method="POST" action="{{ route('productos.actualizar', ['producto'=> $producto->id])}}">
        ...
        <div class="form-row">
            ...
            <input class="form-control" type="text" name="title"
                value="{{ old('title') ?? $producto->title}}" required>
        ...
        <div class="form-row">
            ...
            <select class="custom-select" name="status" required>
                <option {{ old('status') == 'available' ? 'selected' : ($producto->status == 'available' ? 'selected' : '') }} value="available">Available</option>
                <option {{ old('status') == 'unavailable' ? 'selected' : ($producto->status == 'unavailable' ? 'selected' : '') }} value="unavailable">Unavailable</option>
            </select>    
        </div>
        <div class="form-row">
